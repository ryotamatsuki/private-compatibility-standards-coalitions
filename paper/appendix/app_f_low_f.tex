...
\end{equation}
Since $F>0$, every country strictly prefers IS to either non-IS continuation in this sufficient low-cost region. Under the paper's maintained strict-blocking rule, the grand coalition therefore blocks every SU and SW partition, while a deviation from IS to either type of non-IS partition makes each deviator weakly worse and in the relevant roles strictly worse. Thus the low-$F$ comparison is consistent with IS being the unique strict-blocking stable partition on the sufficient region \eqref{eq:appendix-low-f-condition}.
