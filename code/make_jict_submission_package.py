...
def transform_reviewer_dependency(text: str) -> str:
    # The canonical paper remains unchanged. For JICT's page cap, display-heavy
    # equilibrium bookkeeping is moved to the anonymous Online Supplement.
...
\end{align}
Here $P$ is the complete-compatibility profit; $A$ and $B$ are the member and
outsider profits in an SU member market; $C$ and $D$ are the member and native
outsider profits in the outsider market; and $S$ is a foreign-firm profit
...
The detailed derivations and remaining SW consumer-surplus block are in
...
\]
be firm $j$'s net gain from supporting an additional standard. A sufficient,
...
\end{equation}
It makes adoption strictly dominant for the outsider and non-adoption strictly
dominant for every member.

In the canonical continuation, define
\begin{equation}
T_A=P-B,\qquad T_U=A-C,\qquad T_W=A-S.
\label{eq:private-threshold-definitions}
\end{equation}
The exact Cournot expressions imply $T_A,T_U,T_W>0$ and $T_W>T_U$. In a
symmetric SU the outsider obtains the bloc-standard gain in two member markets:
\begin{equation}
\Delta_o^{SU}=2T_A-F,
\qquad
a_o=1\iff F<2T_A.
...
\end{equation}
A member's reverse-adoption thresholds are $T_U-F$ and $T_A-F$, while an SW
foreign firm faces $T_W-F$ or $T_A-F$ depending on the rival's action. Full
best-response calculations are in Online Supplement, Appendix C.
...
If $F_L<F^*$, every $F\in(F_L,F^*)$ yields unique outsider-only adoption under
each SU and unique non-adoption under SW. Every $F>F^*$ yields unique
non-adoption under SU and SW.
\end{lemma}

This selects the private continuation used below. The main political result
depends additionally on how much member-market rent survives adaptation.
"""
    for old, new in MAIN_APPENDIX_REFS.items():
        text = text.replace(old, new)
    return text
...
\begin{document}
\maketitle
\noindent This anonymous supplement contains technical derivations and robustness
calculations referenced by the main article. It introduces no additional headline
claim beyond the certified results stated in the main article.
\appendix
